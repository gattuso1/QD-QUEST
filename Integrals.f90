...
pulse = pulse1 * Ed01 * cos(omega01*(t-t01)+phase01) * exp(-1.e0_dp*((t-t01)**2.e0_dp/(width01**2.e0_dp))) + &
        pulse2 * Ed02 * cos(omega02*(t-t02)+phase02) * exp(-1.e0_dp*((t-t02)**2.e0_dp/(width02**2.e0_dp))) + &
        pulse3 * Ed03 * cos(omega03*(t-t03)+phase03) * exp(-1.e0_dp*((t-t03)**2.e0_dp/(width03**2.e0_dp)))
...
do i=0,nstates-1             
k1(i) = k1(i) - dcmplx(timestep,0.e0_dp) * sum(dcmplx(0.e0_dp,Ham(i,:) - TransHam(i,:)*tp1)* &
xc(:,t)) 
enddo

do i=0,nstates-1
k2(i) = k2(i) - dcmplx(timestep,0.e0_dp) * sum(dcmplx(0.e0_dp,Ham(i,:) - TransHam(i,:)*tp2)* &
(xc(:,t) + k1(:)/9.e0_dp))
enddo

do i=0,nstates-1
k3(i) = k3(i) - dcmplx(timestep,0.e0_dp) * sum(dcmplx(0.e0_dp,Ham(i,:) - TransHam(i,:)*tp3)* &
(xc(:,t)+(k1(:)+3.e0_dp*k2(:))/24.e0_dp))
enddo

do i=0,nstates-1
k4(i) = k4(i) - dcmplx(timestep,0.e0_dp) * sum(dcmplx(0.e0_dp,Ham(i,:) - TransHam(i,:)*tp4)* &
(xc(:,t)+(k1(:)-3.e0_dp*k2(:)+4.e0_dp*k3(:))/6.e0_dp))
enddo

do i=0,nstates-1
k5(i) = k5(i) - dcmplx(timestep,0.e0_dp) * sum(dcmplx(0.e0_dp,Ham(i,:) - TransHam(i,:)*tp5)* &
(xc(:,t)+(-5.e0_dp*k1(:)+27.e0_dp*k2(:)-24.e0_dp*k3(:)+6.e0_dp*k4(:))*0.125e0_dp))
enddo

do i=0,nstates-1
k6(i) = k6(i) - dcmplx(timestep,0.e0_dp) * sum(dcmplx(0.e0_dp,Ham(i,:) - TransHam(i,:)*tp6)* &
(xc(:,t)+(221.e0_dp*k1(:)-981.e0_dp*k2(:)+867.e0_dp*k3(:)-102.e0_dp*k4(:)+k5(:))/9.e0_dp))
enddo

do i=0,nstates-1
k7(i) = k7(i) - dcmplx(timestep,0.e0_dp) * sum(dcmplx(0.e0_dp,Ham(i,:) - TransHam(i,:)*tp7)* &
(xc(:,t)+(-183.e0_dp*k1(:)+678.e0_dp*k2(:)-472.e0_dp*k3(:)-66.e0_dp*k4(:)+80.e0_dp*k5(:)+3.e0_dp*k6(:))/48.e0_dp))
enddo

do i=0,nstates-1
k8(i) = k8(i) - dcmplx(timestep,0.e0_dp) * sum(dcmplx(0.e0_dp,Ham(i,:) - TransHam(i,:)*tp8)* &
(xc(:,t)+&
...
!write(Pop_c_L_f,form_pop_L) time*t_au, dreal(xc_L(0,t)), dreal(xc_L(10,t)), dreal(xc_L(20,t)), &
!                            dreal(xc_L(30,t)), dreal(xc_L(40,t)), dreal(xc_L(50,t)), dreal(xc_L(60,t)), dreal(xc_L(70,t)), &
!                            dreal(xc_L(80,t)), cnorm2
